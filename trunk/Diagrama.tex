...
\begin{tikzpicture}[node distance=3.5cm, every edge/.style={link}]
    ...
    \node[relationship] (prodx) [below=of proggrab] {Producido por} edge node[auto,swap] {1,1} (proggrab);
    
    \node[entity] (productorind) [below=of prodx] {Productor independiente} edge node[auto,swap] {1,N} (prodx);
    ...
    \node[relationship] (realien) [below=of progvivo] {Realizado en} edge node[auto,swap] {1,N} (progvivo);
    
    \node[entity] (est) [below=of realien] {Estudio} edge node[auto,swap] {0,N} (realien);
